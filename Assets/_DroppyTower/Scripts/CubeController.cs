...
//                            SoundManager.Instance.PlaySound(SoundManager.Instance.drop);
                    ...
                    else
                    {
                        ...
                        oldCube = col.gameObject;
                        cubeRigid.linearVelocity = Vector3.zero;
                        cubeRigid.mass = 10;
                        cubeRigid.constraints = RigidbodyConstraints.None;
                        cubeRigid.AddForce(Vector3.down * 3700);
                        cubeRigid.linearVelocity = new Vector3(failForceX, 0, -40);
                        cubeRigid.useGravity = true;
                        cubeRigid.angularDamping = 6f;
                        fail = true;
...
//                    SoundManager.Instance.PlaySound(SoundManager.Instance.drop);
        ...
        private void OnJointBreak(float breakForce)
        {
            ...
            if (!breakFail)
            {
                ...
                cubeRigid.useGravity = true;
                cubeRigid.angularDamping = 6;
                cubeRigid.linearVelocity = new Vector3(reverseForce, 0, -20);
        ...
        void Update()
        {
            ...
            if (breakFail)
            {
                if (cubeRigid.linearVelocity.magnitude > limitVelocity.magnitude)
                {
                    cubeRigid.linearVelocity = new Vector3(reverseForce, -20, -20);
                }
            }
            ...
            if (fail && oldCube != null)
            {
                ...
                {
                    cubeRigid.linearVelocity = new Vector3(failForceX, -30, failForceY);
                }
                else
                {
                    cubeRigid.linearVelocity = new Vector3(failForceX, -30, failForceY);
            ...
            {
                if (cubeRigid.linearVelocity.y > 0)
                    cubeRigid.linearVelocity = new Vector3(cubeRigid.linearVelocity.x, 0, cubeRigid.linearVelocity.z);
                if (Input.GetMouseButtonDown(0) && isNewCube)
                {
                    ...
                    cubeRigid.linearVelocity = new Vector3(0, -70, 0);
                    isNewCube = false;
                    previous = true;
